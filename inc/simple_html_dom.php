...
function file_get_html_curl($url, $options=array(), $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
    // We DO force the tags to be terminated.
    $dom = new simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
    // For sourceforge users: uncomment the next line and comment the retreive_url_contents line 2 lines down if it is not already done.
    $contents = $dom->get_html_curl($url, $options);
...
class simple_html_dom
{
    ...
    {
        $args = func_get_args();
        $this->load(call_user_func_array(array('simple_html_dom', 'get_html_curl'), $args), true);
    ...
    function get_html_curl($url, $options)
    {
        $curl = curl_init($url);
